...
@Component({
  ...
  template: `
    <div class="h-full flex flex-col">
      ...
      <div class="p-6 border-b border-gray-200 bg-white">
        <div class="flex items-center justify-between mb-4">
          <div>
            ...
            <p class="text-gray-600">Adjust stock levels and track movements</p>
          </div>
        </div>
      ...
      <div class="flex-1 overflow-y-auto p-6">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
          ...
          <div class="p-6">
            ...
            <div *ngIf="inventoryService.allStockMovements().length > 0" class="space-y-3">
              <div
                *ngFor="let movement of inventoryService.allStockMovements().slice(0, 10)"
                class="flex items-center justify-between p-4 bg-gray-50 rounded-lg"
              >
                <div class="flex items-center gap-3">
                  <div [class]="'w-10 h-10 rounded-lg flex items-center justify-center ' + 
                               (movement.type === 'in' ? 'bg-green-100' : movement.type === 'out' ? 'bg-red-100' : 'bg-blue-100')">
                    <lucide-icon 
                      [img]="movement.type === 'in' ? plusIcon : movement.type === 'out' ? minusIcon : editIcon" 
                      [class]="'w-5 h-5 ' + 
                               (movement.type === 'in' ? 'text-green-600' : movement.type === 'out' ? 'text-red-600' : 'text-blue-600')">
                    </lucide-icon>
                  </div>
                  <div>
                    <div class="font-medium text-gray-900">{{ movement.productName }}</div>
                ...
                <div class="text-right">
                  ...
                  <div class="text-xs text-gray-500">{{ movement.timestamp | date:'short' }}</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200">
          <div class="p-6 border-b border-gray-200">
            <h3 class="text-lg font-semibold text-gray-900">Product Stock Levels</h3>
          </div>
            ...
            <table class="w-full">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <tr *ngFor="let product of filteredProducts(); trackBy: trackByProductId" class="hover:bg-gray-50">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    <span [class]="'text-lg font-bold ' + getStockStatusColor(product)">
                      {{ product.stock }}
                    </span>
                  </td>
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    <span *ngIf="product.stock === 0" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
                      <lucide-icon [img]="alertIcon" class="w-3 h-3 mr-1"></lucide-icon>
                      Out of Stock
                    </span>
                    <span *ngIf="product.stock > 0 && product.stock <= product.minStock" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-orange-100 text-orange-800">
                      <lucide-icon [img]="alertIcon" class="w-3 h-3 mr-1"></lucide-icon>
                      Low Stock
                    </span>
                    <span *ngIf="product.stock > product.minStock" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                      In Stock
                    </span>
                  </td>
    ...
    <div *ngIf="adjustingProduct()" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        <div class="p-6 border-b border-gray-200">
          <h3 class="text-xl font-bold text-gray-900">Adjust Stock</h3>
          <p class="text-gray-600 mt-1">{{ adjustingProduct()?.name }}</p>
        </div>

        <form (ngSubmit)="saveStockAdjustment()" class="p-6 space-y-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Current Stock</label>
            <input
              type="number"
              [value]="adjustingProduct()?.stock"
              readonly
          ...
          <div>
            ...
            <input
              type="number"
              [(ngModel)]="adjustmentForm.newStock"
              name="newStock"
              min="0"
              required
          ...
          <div *ngIf="adjustmentForm.reason === 'Other'">
            <label class="block text-sm font-medium text-gray-700 mb-1">Custom Reason</label>
            <input
              type="text"
              [(ngModel)]="adjustmentForm.customReason"
              name="customReason"
              class="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
          </div>
          ...
          <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <div class="flex justify-between items-center">
              ...
              <span [class]="'font-bold ' + (getAdjustmentDifference() >= 0 ? 'text-green-600' : 'text-red-600')">
                {{ getAdjustmentDifference() >= 0 ? '+' : '' }}{{ getAdjustmentDifference() }}
              </span>
            </div>
          </div>
