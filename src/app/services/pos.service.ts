...
export class PosService {
  ...
  addToCart(product: Product, quantity: number = 1) {
    ...
    const currentProduct = this.inventoryService.allProducts().find(p => p.id === product.id);
    if (!currentProduct) {
    ...
    if (currentProduct.stock < totalRequiredQuantity) {
      throw new Error(`Insufficient stock. Available: ${currentProduct.stock}, Required: ${totalRequiredQuantity}`);
    }
  ...
  updateQuantity(productId: string, quantity: number) {
    ...
    if (item) {
      if (quantity <= 0) {
        ...
        const currentProduct = this.inventoryService.allProducts().find(p => p.id === productId);
        if (!currentProduct) {
          throw new Error('Product not found');
        }

        if (currentProduct.stock < quantity) {
          throw new Error(`Insufficient stock. Available: ${currentProduct.stock}, Required: ${quantity}`);
        }
  ...
  processOrder(paymentMethod: 'cash' | 'card' | 'digital', customerName?: string): Order {
    ...
    cartItems.forEach(item => {
      ...
      this.inventoryService.updateProductStock(item.product.id, item.quantity, 'out');
      ...
      const updatedProduct = this.inventoryService.allProducts().find(p => p.id === item.product.id);
